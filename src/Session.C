...
void Db_session::configure_auth() { services.configure(); }
...
dbo::ptr<User> Session::user() const {
  ...
  if (!auth_info_)
    auth_info_ = users().find(login_.user());
  if (!auth_info_)
    return nullptr;
...
std::string Session::user_name() const { return user()->name(); }
...
std::vector<dbo::ptr<User_stats>> Db_session::top_users(int limit) {
  ...
  dbo::collection<dbo::ptr<User_stats>> top = find<User_stats>()
...
const Auth::AuthService &Db_session::auth() { return services.auth; }
...
bool Session::authenticate_from_environment(Environment const &env) {
  ...
  auto auth_user =
      find_from_environment<auth_user_t>(CONFIG.auto_create_accounts, env);
