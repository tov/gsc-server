...
char const *Enum<File_purpose>::show(File_purpose purpose) {
  switch (purpose) {
  case File_purpose::config:
    return "config";
  case File_purpose::log:
    return "log";
  case File_purpose::resource:
    return "resource";
  case File_purpose::source:
    return "source";
  case File_purpose::test:
    return "test";
  case File_purpose::forbidden:
    return "forbidden";
  }
}
...
regex const config_file("Makefile" OR "CMakeLists\\.txt" OR "goal\\.txt" OR
                        ".gitignore" OR EXT("md"),
                        opt);
...
File_meta::File_meta(std::string const &name, std::string const &media_type,
    ...
    : name_{name}, time_stamp_{WDateTime::currentDateTime()},
      line_count_{line_count}, byte_count_{byte_count}, submission_{submission},
      media_type_{media_type}, purpose_{purpose}, uploader_{uploader} {}
...
dbo::ptr<File_meta> File_meta::upload(const string &name, const Bytes &contents,
  ...
  dbo::ptr<File_meta> result = session.addNew<File_meta>(
      name, media_type, purpose, uploader, line_count, byte_count, submission);
...
void File_meta::reclassify(File_purpose purpose) { purpose_ = purpose; }
...
bool operator<(const File_meta &a, const File_meta &b) {
  ...
  // sort by type first:
  if (display_before(a_type, b_type))
    return true;
  if (display_before(b_type, a_type))
    return false;
