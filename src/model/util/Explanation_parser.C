                       ...
                       File_viewer::Highlighter& highlighter)
{
    ...
    for (;;) {
        ...
        if (tok.type == TokenType::eof) {
            ...
            if (tok2.type != TokenType::tag
                && tok2.type != TokenType::digits) {
                lexer.unnext(tok2);
                lexer.unnext(tok1);
                highlighter.highlight(line);
                continue;
...
Token Lexer::real_next_()
{
    ...
    auto consumed = [&]() -> ssize_t {
        return tok.end - tok.begin;
    };
...
WString Explanation_writer::wstr() const
{
    return WString(os_.str());
}

void Explanation_writer::raw_text_(char c)
{
    os_ << c;
}

void Explanation_writer::raw_text_(string_view sv)
{
    os_ << sv;
}
