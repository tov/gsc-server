...
User::User(const std::string &name, Role role)
    : name_(name), role_(static_cast<int>(role)) {}
...
bool User::can_admin() const { return role() == User::Role::Admin; }
...
std::vector<dbo::ptr<Submission>> User::submissions() const {
  ...
  for (const auto &each : submissions1_)
    insert(each);
  for (const auto &each : submissions2_)
    insert(each);
...
std::string User::hw_url() const { return "/~" + name() + "/hw"; }

std::string User::profile_url() const { return "/~" + name() + "/profile"; }

std::string User::rest_uri() const { return api::paths::Users_1(name()); }
...
int User::Role_info::to_repr(Role role) { return static_cast<int>(role); }
