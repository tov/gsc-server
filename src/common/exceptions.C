...
File_too_large_error::File_too_large_error(string filename, int needed,
    ...
    : Html_error{file_too_large_msg_(filename, needed,
      ...
      filename_{move(filename)}, needed_{needed}, allowed_{fix_file_size_limit_(
                                                      allowed)} {}
...
const char *Move_collision_error::title() const { return "Cannot Move File"; }
...
const char *Generic_html_error::title() const { return "Runtime error"; }

ostream &Generic_html_error::write_body_html(ostream &note) const {
  return note << html::Escape{what()};
}

const char *No_partner_to_separate::title() const { return "Separation Error"; }
...
const char *Whose_files_are_these::title() const { return "Separation Error"; }
...
const char *Bad_file_type_error::title() const { return "Bad File Type"; }
...
const char *File_too_large_error::title() const { return "File Too Large"; }
...
Join_collision_error::Join_collision_error(submission_t o1, submission_t o2,
    ...
    : Html_error{join_collision_msg_(o1, o2, filenames)},
      submissions_{move(o1), move(o2)}, filenames_{move(filenames)} {}

Move_collision_error::Move_collision_error(submission_t o1, filename_t s1,
                                           submission_t o2, filename_t s2)
    : Html_error{move_collision_msg_(o1, s1, o2, s2)}, submissions_{o1, o2},
      filenames_{s1, s2} {}

No_partner_to_separate::No_partner_to_separate(string username)
    : Html_error{no_partner_to_separate_msg_(username)}, username_{
                                                             move(username)} {}

Whose_files_are_these::Whose_files_are_these(Source_file_vec files)
    : Html_error{whose_files_are_these_msg_(files)}, lost_files_{move(files)} {}

Bad_file_type_error::Bad_file_type_error(string filename)
    : Html_error(bad_file_type_msg_(filename)), filename_{move(filename)} {}
...
Would_exceed_quota_error::Would_exceed_quota_error(string filename, int needed,
    ...
    : Html_error{would_exceed_quota_msg_(filename, needed, available)},
      filename_{move(filename)}, needed_{needed}, available_{available} {}
