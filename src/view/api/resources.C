...
struct Key_eater {
  ...
  template <class F> void take(std::string const &key, F f) {
...
template <class T> std::unique_ptr<T> match(std::string const &path_info) {
...
void Grades_csv::load_(const Resource::Context &context) {
  ...
  const auto assigns_q = dbo.find<Assignment>().orderBy("number").resultList();
...
public:
  ...
  explicit Users_1(uri_type &&uri) : uri_{std::move(uri)} {}
...
void Users_1::do_get_(Context const &) { use_json(user_->to_json()); }
...
void Users_1::do_patch_(Request_body body, Context const &context) {
  ...
  try {
      ...
      if (pair.first == "role") {
        ...
        for (J::Object const &exam : exams) {
          ...
          if (possible == 0) {
            ...
            nested.success()
                << "Set exam " << number << " grade for " << user_->name()
                << " to " << points << " / " << possible << ".";
          }
        }
        ...
        for (J::Object const &request : requests) {
          ...
          case S::outgoing: {
            ...
            if (incoming && incoming->requestee() == user_) {
              if (incoming->confirm(context.session)) {
                nested.success() << "Requested and confirmed partnership with "
                                 << user_and_hw << ".";
                break;
              }
            }
            ...
            auto request_ptr = Partner_request::create(context.session, user_,
                                                       other, hw, reason);
            if (!request_ptr)
              throw Http_status{403, reason.str()};

            nested.success()
                << "Requested partnership with " << user_and_hw << ".";
            break;
          }

          case S::incoming:
            nested.failure()
                << "You cannot create an incoming partner request.";
            break;
          ...
          case S::accepted: {
            ...
            if (request_ptr) {
              ...
              if (submission) {
                nested.success()
                    << "Accepted partner request from " << user_and_hw << ".";
              } else {
                nested.failure() << "Could not accept partner request from"
                                 << user_and_hw << ".";
              }
            } else {
              nested.failure()
          ...
          case S::canceled: {
            ...
            if (incoming && outgoing) {
              nested.success()
                  << "Removed incoming partner request from " << user_and_hw
                  << " and outgoing partner request to " << user_and_hw << ".";
            } else if (incoming) {
              nested.success() << "Removed incoming partner request from "
                               << user_and_hw << ".";
            } else if (outgoing) {
              nested.success() << "Removed outgoing partner request to "
                               << user_and_hw << ".";
            } else {
              nested.failure() << "No partners requests to remove from/to "
                               << user_and_hw << ".";
            }
    ...
    throw Http_status{400,
                      "PATCH /users/_1 could not parse user request as JSON"};
...
public:
  ...
  explicit Users_1_submissions(uri_type &&uri) : uri_{std::move(uri)} {}
...
public:
  ...
  explicit Submissions_1(uri_type &&uri) : uri_{std::move(uri)} {}
    ...
    for (auto const &pair : object) {
      if (pair.first == "due_date") {
        ...
        if (auto res = submission_.modify()->set_due_date(time_spec);
            res.has_value()) {
          result.success() << "Modified due date to " << res->toString()
                           << " UTC";
        } else {
        ...
        if (auto res = submission_.modify()->set_eval_date(time_spec);
            res.has_value()) {
          result.success() << "Modified eval date to " << res->toString()
                           << " UTC";
        } else {
          result.failure() << "Could not parse timespec ‘" << time_spec << "’";
        }
...
public:
  ...
  explicit Submissions_1_files(uri_type &&uri) : uri_{std::move(uri)} {}
...
public:
  ...
  explicit Submissions_1_files_2(uri_type &&uri) : uri_{std::move(uri)} {}
...
void Submissions_1_files_2::do_patch_(Request_body body,
  ...
  try {
    ...
    processor.take("name", [&](std::string const &value) { opt_name = value; });
    ...
    processor.take("media_type",
                   [&](std::string const &value) { opt_media_type = value; });
    ...
    throw Http_status{
        400, "PATCH /submissions/_1/files/_2 could not understand request"};
  } catch (J::ParseError const &e) {
    throw Http_status{
        400,
        "PATCH /submissions/_1/files/_2 could not parse user request as JSON"};
  } catch (std::invalid_argument const &e) {
    throw Http_status{400, e.what()};
  }
...
void Submissions_1_files_2::do_put_(Request_body body,
  ...
  auto file_meta = File_meta::upload(
      uri_.filename, std::move(body).read_bytes(), submission_, context.user);
...
public:
  ...
  explicit Submissions_1_evals(uri_type &&uri) : uri_{std::move(uri)} {}
...
public:
  ...
  explicit Submissions_1_evals_2(uri_type &&uri) : uri_{std::move(uri)} {}
...
public:
  ...
  explicit Submissions_1_evals_2_self(uri_type &&uri) : uri_{std::move(uri)} {}
...
void Submissions_1_evals_2_self::do_put_(Request_body body,
  ...
  try {
    ...
    throw Http_status{
        400, "PUT /submissions/_1/evals/_2/self could not understand request"};
  } catch (J::ParseError const &e) {
    throw Http_status{400, "PUT /submissions/_1/evals/_2/self could not parse "
...
void Submissions_1_evals_2_grader::do_delete_(
    Resource::Context const &context) {
...
public:
  ...
  explicit Submissions_hw1(uri_type &&uri) : uri_{std::move(uri)} {}
...
void Submissions_hw1::load_(Context const &context) {
  ...
  std::copy(assignment->submissions().begin(), assignment->submissions().end(),
...
void Whoami::load_(const Resource::Context &context) { user_ = context.user; }

void Whoami::do_get_(const Resource::Context &) {
  content_type = "text/plain";
  if (user_)
    contents = Bytes(user_->name());
}
...
void Resource::do_delete_(Context const &) { not_supported(); }

void Resource::do_get_(Context const &) { not_supported(); }

void Resource::do_patch_(Request_body, Context const &) { not_supported(); }

void Resource::do_post_(Request_body, Context const &) { not_supported(); }

void Resource::do_put_(Request_body, Context const &) { not_supported(); }
