...
{
    return initial_value_ != optional_value()
           || explanation_has_changed();
}
...
void Explanation_response_widget::set_initial_value(
        double v)
{
    set_value(v);
...
bool Boolean_response_widget::is_valid() const
{
    ...
    return checked == no_
        || (checked == yes_ && !explanation_is_empty());
}
...
void Scale_response_widget::freeze_value()
{
    slider_->disable();
}
...
bool Informational_response_widget::has_changed() const
{ return true; }

bool Informational_response_widget::is_valid() const
{ return true; }
