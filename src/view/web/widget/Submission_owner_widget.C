...
Submission_owner_widget::Submission_owner_widget(Submission_context const& context)
...
void Submission_owner_widget::update_student_()
{
    ...
    if (submission()->user2()) {
        auto other_user = self == submission()->user1()
                          ? submission()->user2()
                          : submission()->user1();
    ...
    impl_->addNew<Partner_notification_widget>(self, submission(), session(), changed());
}
