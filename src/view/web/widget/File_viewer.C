    ...
    : file_meta_(source_file)
{
    ...
    if (numbered) { table_->setHeaderCount(1, Orientation::Vertical); }

    auto file_head = table_->elementAt(0, 0);
    if (numbered) { file_head->setColumnSpan(2); }
...
void File_viewer::reload_()
{
    ...
    for (const auto& file : files) {
        if (file->media_type() == "text/plain") {
            file_contents_->addNew<Plain_text_file_viewer>(file, line_number,
                                                           lines_, this);
        }
