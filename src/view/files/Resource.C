                             ...
                             Response& response, Session& session)
{
    ...
    if (! owner) not_found("No such user");

    auto submission = Submission::find_by_assignment_number_and_user(
            session, parsed.assignment, owner);
    if (! submission) not_found("No such submission");

    if (! submission->can_view(session.user())) denied("That isn't yours");

    auto file_meta = submission->find_file_by_name(parsed.filename);
    if (! file_meta) not_found("No such file");

    std::unique_ptr<File_data> file_data =
            std::make_unique<File_data>(file_meta);

    if (! file_data->populate_contents()) not_found("File data absent");
